...
export  function pointInPolygon(point: any, lines: any[]) {
  let inside = false;
  for (let i = 0; i < lines.length-1; i++) {
    let slope = (lines[i+1][1] - lines[i][1]) / (lines[i+1][0] - lines[i][0]);
    let xBetween = (lines[i][0] < point[0] && point[0] < lines[i+1][0])
      || (lines[i+1][0] < point[0] && point[0] < lines[i][0]);
    let yAbove = point[1] < (slope * (point[1] - lines[i][1]) + lines[i][1]);
    if (xBetween && yAbove) {
      inside = !inside;
    }
  }
...
export async function listByName(params: any) {
  return request(`/jetlinks/device-instance/device_name/${params}`, {
    method: 'POST',
    params,
  });
}
export async function listByProductId(params: any) {
  return request(`/jetlinks/device-instance/list/${params}`, {
    method: 'POST',
    params,
  });
}
