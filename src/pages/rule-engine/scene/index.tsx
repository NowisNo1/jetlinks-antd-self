...
const RuleInstanceList: React.FC<Props> = props => {
  ...
  const initState: State = {
    data: result,
    searchParam: {
      pageSize: 8, terms: { modelType: 'rule-scene' }, sorts: {
        order: "descend",
        field: "createTime"
      }
    },
          ...
          <div>
            <SearchForm
              ...
              search={(params: any) => {
                setSearchParam(params);
                handleSearch({
                  terms: params && { modelType: 'rule-scene' }, pageSize: 8, sorts: searchParam.sorts || {
                    order: "descend",
                    field: "createTime"
                  }
                });
      ...
      <div>
        {sceneList.data && (
          <List<any>
            ...
            renderItem={item => {
              if (item && item.id) {
                let tri = JSON.parse(item.modelMeta);
                let ans = "";
                if(tri.type !== undefined && tri.type.length > 0){
                  console.log(tri.type[0].trigger);
                  ans = tri.type[0].trigger;
                }
                  return (
